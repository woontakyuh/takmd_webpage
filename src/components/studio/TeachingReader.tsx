...
export function TeachingReader({ presentations, selected, onSelect, slideIndex, onSlide, updatedAt }: Props) {
  ...
  return <div className="teaching-reader" data-active-talk={selected?.id}>
    {selected ? <>
      <nav className="folio-paging" aria-label="Browse presentations"><span>Presentation {navigation.index + 1} / {navigation.total}</span><div>
      ...
      {slide ? <>
        <div className="folio-paging"><span role="status">{slideIndex + 1} / {slides.length} · On the wall TV</span><div>
        ...
        <figure className="talk-slide" key={slide.src}><button className="slide-open" onClick={() => setViewerOpen(true)} aria-label="Open presentation slide viewer"><img src={slide.src} alt={slide.caption} width={slide.width ?? 1280} height={slide.height ?? 720} decoding="async" /><span>View larger ↗</span></button><figcaption>{slide.caption}</figcaption></figure>
        {slides.length > 1 && <nav className="slide-strip" aria-label="Presentation slides">{slides.map((item, index) => <button key={item.src} aria-label={'Show presentation slide ' + (index + 1)} aria-pressed={index === slideIndex} onClick={() => onSlide(index)}><img src={item.thumbnail ?? item.src} alt="" width={160} height={90} loading="lazy" decoding="async" /><span>{String(index + 1).padStart(2, '0')}</span></button>)}</nav>}
        <p className="studio-meta">{media?.kind === 'full' ? 'Complete presentation' : 'Selected slides'} · The wall TV follows your selection.</p>
      ...
      <p className="studio-panel-intro">Select a presentation to put it on the wall TV.</p>
    ...
    {viewerOpen && selected && <SlideViewer title={selected.topic || selected.title} slides={slides} index={slideIndex} onSlide={onSlide} onClose={() => setViewerOpen(false)} />}
  </div>;
}
