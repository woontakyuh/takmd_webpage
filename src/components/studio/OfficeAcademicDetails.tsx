...
export function EducationDetails({ onTalk }: { readonly onTalk: (id: string) => void }) {
  ...
  return <>
    ...
    {programs.map(program => <section className="studio-editorial-note" key={program.title}><span>{program.tags.join(' / ')}</span><h3>{program.title === 'ESS Workshop for Beginners' ? <a href="/workshops">{program.title} ↗</a> : program.title}</h3><p>{program.description}</p></section>)}
