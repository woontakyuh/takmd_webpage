...
import { restoreMagazinePrint } from './MagazinePrint';
import { PALETTE } from './config';
import { createMagazineLeafGeometry, magazineLeafPose, shapeMagazineLeaf } from './MagazineLeafGeometry';
import type { MagazineLeafShape } from './MagazineLeafGeometry';

function PrintedMaterial({ surface, attach, cover, reading }: {
  readonly surface: MagazineSurface;
  ...
  return <meshStandardMaterial attach={attach} map={texture} color={surface.albedo ?? PALETTE.white}
    ...
    emissiveIntensity={reading ? .06 : 0} toneMapped={false} onUpdate={material => restoreMagazinePrint(material, surface)} />;
}

export function MagazineLeaf({ shape, front, back, cursor, openingCursor = cursor, leafIndex, cover = false, reading = false, handlers, name }: {
  ...
  readonly openingCursor?: RefObject<number>;
  ...
  const lastOpeningCursor = useRef(-1);
  const { geometry, flat } = useMemo(() => {
    ...
    return created;
  }, [shape, front, back, cursor, openingCursor, leafIndex, cover]);
  ...
  useEffect(() => {
    ...
    lastOpeningCursor.current = -1;
    return () => geometry.dispose();
  }, [geometry]);
  useEffect(() => () => edgeTexture.dispose(), [edgeTexture]);
  useFrame(() => {
    if (Math.abs(cursor.current - lastCursor.current) < .00001
      && Math.abs(openingCursor.current - lastOpeningCursor.current) < .00001) return;
    ...
    <mesh geometry={geometry} castShadow receiveShadow>
      ...
      {front ? <PrintedMaterial surface={front} attach="material-4" cover={cover} reading={reading} />
        : <meshStandardMaterial attach="material-4" color={PALETTE.paperLight} roughness={.94}
          emissive={PALETTE.paperLight} emissiveIntensity={reading ? .06 : 0} toneMapped={false} />}
