...
export function DeskAccessories({ selected, onSelect, reducedMotion, onClaudeSticker }: Pick<StudioSceneProps, 'selected' | 'onSelect' | 'onClaudeSticker' | 'reducedMotion'>) {
  return (
    <group>
      ...
      <Interactive id="projects" selected={selected} onSelect={onSelect} reducedMotion={reducedMotion} position={[0.19, DESK_MAT_TOP, -0.17]}><MacMini position={[0, 0, 0]} onClaudeSticker={onClaudeSticker} /></Interactive>
    </group>
  );
}
