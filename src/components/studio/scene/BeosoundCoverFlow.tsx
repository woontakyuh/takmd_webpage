...
type Props = { readonly album: AlbumId; readonly onAlbum: (album: AlbumId) => void; readonly onOpen: () => void };

export function BeosoundCoverFlow({ album, onAlbum, onOpen }: Props) {
  ...
  return <div className="cd-browser cd-cover-flow">
    ...
    <div className="cd-case-rail" aria-label="Browse album cases" data-dragging={dragOffset !== 0}
      ...
      onPointerMove={event => {
        ...
        if (!start || start.id !== event.pointerId || Math.abs(event.clientX - start.x) < 8) return;
        swiped.current = true;
      ...
      {ALBUM_IDS.map((id, slot) => {
        const offset = slot - index, distance = Math.abs(offset), record = BEOSOUND_ALBUMS[id];
        const style = {
        ...
        return <button type="button" key={id} style={style} aria-label={`Browse ${record.artist} — ${record.album}`}
