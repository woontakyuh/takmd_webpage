...
function OfficeExperience(content: StudioContent) {
  ...
  return <div className="studio" data-entry={entry} data-guided={guidedSection ?? undefined} data-night={night} data-selected={selected ?? focused ?? (details ? 'details' : undefined)} data-reading={selected ?? undefined} data-approached={focused ?? undefined} data-inspecting={inspection ? 'whisky' : undefined} data-explored={explored} data-arranging={arrangement.editing}>
    ...
    {!zoomed && !selected && focused && !details && <div className="office-approach-actions"><button className="studio-icon-button" onClick={close} aria-label="Return to previous office view"><OfficeIcon name="close" /></button><button onClick={() => open(focused)}>{focused === 'spine' ? 'UBE & Teaching' : <>Open {focused === 'ai' ? 'monitor' : focused === 'education' ? 'TV' : 'object'}</>}</button></div>}
