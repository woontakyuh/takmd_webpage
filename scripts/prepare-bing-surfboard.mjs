...
const usage='bun scripts/prepare-bing-surfboard.mjs /absolute/path/to/private/Down [output.glb]';
const [sourceDirectory,output='public/models/surfboard.glb']=process.argv.slice(2);
if(sourceDirectory==='--help'){console.log(usage);process.exit(0);}
assert(sourceDirectory,usage);
const evidence='.omo/evidence/bing-fidelity-2026-09-11';
await mkdir(evidence,{recursive:true});
const textureOutput=await createBingTextures(resolve(sourceDirectory));
...
const json={asset:{version:'2.0',generator:'TakMD reference-derived Bing surfboard'},scene:0,
  ...
  extensionsRequired:['EXT_texture_webp'],extras:{lengthMetres:BING_BOARD.length,
    widthAndThickness:'Visual reconstruction from oblique owner photographs; not manufacturer measurements',
...
const materials={
  ...
  fin:{name:'Pale ivory fiberglass single fin',pbrMetallicRoughness:{baseColorFactor:[.79,.87,.71,1],metallicFactor:0,roughnessFactor:.23},
    extensions:{KHR_materials_transmission:{transmissionFactor:.42},KHR_materials_volume:{thicknessFactor:.007,attenuationColor:[.94,.98,.87],attenuationDistance:.05},KHR_materials_ior:{ior:1.49},KHR_materials_clearcoat:{clearcoatFactor:.65,clearcoatRoughnessFactor:.16}}},
