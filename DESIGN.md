...
Use a warm roughly 3000 K task pool (the existing #FFD29A warm-light token) directed downward and forward to the keyboard, leaving the printed folio darker outside the pool, plus a restrained local rear fill around the monitor. Neither source may illuminate the monitor panel directly. Both light intensities must be multiplied by `sun.lamp`, so the room keeps its darker evening character and the bar is unlit in daylight. The physical diffusers may be subtly emissive to identify the fixture, but no bloom, unbounded white plane, purple tint, or fake glow card is permitted. Keep the floor AJ lamp and concealed warm shelf wash; do not restore the removed desk AJ lamp. Reuse the existing cached-shadow renderer and keep the front shadow map modest.
