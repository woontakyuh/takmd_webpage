...
The current model provides disc selection, cover movement and transport/volume state, but no source audio is loaded. Play must report that honestly. The owner will choose six albums; Asoto Union and Brown Eyes Reason 4 Breathing are stated favorites, not an authorized final playlist. Audio must be loaded only after an intentional playback action and must use a source permitted for website playback. Never rip or hide a YouTube player to simulate background audio, or imply the owner's streaming subscription can serve all visitors.

### Photographed Bing surfboard — 2026-09-11

The six newly supplied Down photos supersede the earlier generated board graphics. Preserve the actual amber board's separate deck/bottom lettering, 60th-anniversary badge, wax region, rails and single pale fin/fin box. Its 9ft6in room length remains; width, thickness and unseen profile are reference-based estimates rather than claimed factory measurements. Use the source-derived lettering, not retyped or fabricated lettering. Originals stay unchanged in Dropbox; the generated GLB and reproducible preparation scripts record provenance.
