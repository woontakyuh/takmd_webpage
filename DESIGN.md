...
## Physical interaction refinements — 2026-09-10 evening
Cabinet handle and open edge have no visible marker; retain physical pointer hit targets and keyboard access. The A3 lecture sheet has a quiet 4-degree in-plane tilt, asymmetrical steel magnets, shallow paper creases and a slightly curled free lower corner. Paper uses existing paper/light/aluminium materials; keep source content unchanged and flatten the same sheet for its sharp focused reading surface. No idle paper motion or new assets.
Bottle routes include the unfolded worktop and its underside lip, not only the bottle shelves: lower bottles withdraw, clear the front worktop edge, rise and settle. Exact reversed routes return them. Bottle separation remains horizontal to preserve shelf/worktop heights. Concurrent switches remain immediate; reduced motion lands on exact endpoints.
The whole focused gold plaque opens the existing Short, matching its full-body hover; distant activation still frames the photo/plaque pair. The TV shows one persistent archive/slide composition before and after focus, with a compact year/date tree. Any personal book starts closed on its cover regardless of available pages. Move the Halo2 controller beside the Mac mini.

Research reading prioritizes the printed right page, substantially larger on desktop and mobile. The blank left cover may extend beyond the viewport; retain a visible inner portion for previous-page clicks. Keep the printed page clear of the details panel and inside the viewport. On mobile, reserve the upper two-thirds for the book, tilt the reading camera more toward the paper, and limit the scrollable details panel to 32dvh; its expanded mode remains available. Use responsive camera framing, preserving the physical book, turning animation and Overview return.

## Room-native readers and collections — 2026-09-11

This section supersedes earlier always-mounted screen DOM, automatic cabinet opening, two-magnet lecture, and boxed object-description rules.

- Resting TV and monitor content belongs to depth-tested WebGL screen planes. Mount the sharp interactive DOM only after the selected camera pose settles, and hide the underlying screen plane in the same render. Exit restores only the physical screen, keeping the selected TV slide or CV scroll slice. The original CV layout and loading-to-live continuity remain canonical.
- A closed Isidoro requires approach, then a deliberate second physical click to open. Its real left free edge closes it. No grip dot or visible boxed instruction is used. Keyboard affordances remain available as plain focus text; approach actions have no background panel.
- The A3 whisky lecture has one dimensional upper-left magnet, a four-degree casual tilt, and a small downward/right placement offset. Right-sheet and parked-left-sheet clicks turn forward/back around the fixed corner, with bounded spring motion and an immediate reduced-motion state. Only nearby page faces mount. X/Escape returns to the closed cabinet. There is no View on TV button.
- The three credentials form one inspectable group, as do the three right-shelf awards. First click fits the whole group to the viewport; selecting an item next frames it with readable, unboxed English copy beside the projected object. The complete focused frame remains visible. On desktop, SNU copy belongs at the upper right, above its landscape neighbor. Use a compact measured stack only when the available width cannot fit adjacent copy; portrait copy sits beneath the object and scrolls within its allotted area. Keep neighboring items clickable. X returns to the pre-collection pose, and Overview remains accessible.
- Editorial explanatory type uses `--studio-editorial: 'Iowan Old Style', 'Palatino Linotype', 'Book Antiqua', Georgia, serif`. Body copy is 14–16px at 1.55–1.65 line height; item titles are 24–32px. No extra font request is added. Original document/slide languages and the Ppomppu Easter egg remain unchanged.
- The clock opens a paper monthly conference calendar beside the real object, or below it on a narrow screen. The calendar uses a warm fixed paper palette, existing ink/muted/clay/teal values scoped independently of night scene overrides, seven weekday columns and an accessible event ledger. Month-edge and keyboard navigation, scrollable long content and a plain 44px X control are required. The supplied public snapshot includes faculty/attendance, not only presentations.
- Both physical wall switch centers are 1.22m high. Blind controls remain left of the window. Matte warm-gray plates and darker arrows distinguish controls from plaster without adding saturated accents.
- Entry posters are build-generated snapshots of the current office, with four camera/aspect classes and day/night variants. They share the live camera projection and centered crop, including the early-CV monitor origin. Freeze the actual build date and use explicit lighting previews so date-dependent TV defaults match. Source/image hashes gate freshness; unchanged releases reuse their images. Clock ticks and continuous real-time light are not promised to be pixel-identical to a still.

The speaker feasibility brief below is superseded by the owner's later approval of Beosound 9000 with a silver/black Beolab 8000 pair. Actual audio and the six-album library remain a separate owner-dependent decision. Further rendering-cost and Han River fidelity work remains explicitly tracked in the follow-up report.

### Whisky paper bend correction — 2026-09-11

The whisky lecture is 26 separate A3 sheets held at the existing upper-left corner by one dimensional 22mm nickel pot magnet. Each sheet is 0.12mm thick; advancing transfers one leaf from the right stack to the left stack and reversing restores it. Keep the printed right sheet readable and the turned left sheet directly clickable. On portrait screens the outer blank left stack may extend beyond the viewport while the complete printed right sheet and a useful previous-page strip stay visible. No page badges, text navigation, floating reader box or TV link.

Use a segmented 64 × 24 WebGL sheet with separate printed front and ivory reverse. A continuous cylindrical bend sweeps outward around the magnet area, with shallow free-edge sag and no modeled book spine. The corner remains fixed; the finite-radius fold is a visual sheet approximation, not a cloth simulation. Movement follows the existing 8/second object damping, capped at 2.8 pages/second for rapid input, and retargets from its current position. Reduced motion lands directly on the requested state. The beui.dev cylinder-carousel source informed the continuous retargetable cursor; the page deformation is specific to the cabinet.

All 26 paper layers are conserved, including the moving sheet. Allocate HD original-slide textures only for the current page and its immediate neighbors, and dispose owned textures and geometry on eviction or close. The original public slide source selection remains authoritative. Preserve the cabinet approach requirement, arrow-key navigation, 44px native X, Escape and Overview.

### Spine Summit photograph boards — 2026-09-11

Use a 1200 × 800 logical gallery board with 24px mat and 16px gutters in the existing paper-light/ink palette. DOM and canvas share normalized image slots. Retain all eight originals, grouped into three compositions: photographs 1 and 2 stacked beside portrait 3; photograph 4 as the hero with 5 and 6; photographs 7 and 8 as a pair. Selecting a photograph enlarges it within the physical TV experience and preserves X/Overview return without an external redirect.

### Screen-responsive TV backlight — 2026-09-11

The existing four rear strips follow the actual left, top, right and bottom image regions. Exclude the lecture archive, footer and outer photo mat from the source region; preserve the composite photograph arrangement. Sample static media only on image changes through one cached 48 × 32 CPU canvas with 20%-deep edge bands and at most 32 cached sources. An enlarged photograph overrides the gallery sample until returning. The TV currently has no native video surface; do not infer or sample external iframe content.

Use source luminance for light intensity, with black at zero and white as a restrained #FFF5E6 warm neutral. Saturated image regions retain their hue. Update the existing four area lights and strip emissive materials with exponential damping at 10/second (95% settled at approximately 300ms); reduced motion applies the new static state immediately. Retain hover/focus brightness and the existing four-light geometry without adding lights, shadows or GPU readback. Left/right follows the screen as viewed from the room, accounting for its rotated physical plane.

### Object inspection composition — 2026-09-11 afternoon

Descriptions belong beside the projected object, not at a fixed viewport edge. Use measured frame bounds for credential, plaque and family-photo copy; SNU text occupies the upper-right space above the adjacent landscape frame. Bottle information uses the left side of its cabinet to avoid the guitar. Portrait descriptions remain below their objects. KOMISS paper uses a document-only UV perspective correction so the photographed inner border fits the rectangular frame; original writing, signatures and seal remain unchanged.

Only the currently inspected object's information remains mounted. Inspection clips and pins the outer document; it must not become a hidden scroll container when a native control receives focus. Calendar and CV inner content must retain independent scrolling, and closing the inspection restores ordinary page scrolling. Month navigation occupies fixed positions outside the variable event ledger. Multi-day calendar entries span adjacent day columns, continue across week boundaries and share stable lanes when events overlap.

Room, blind and Halo controllers anchor to their physical objects in desktop screen space, choosing nearby free space and avoiding the window or Mac mini top. Preserve the existing control design and functions. Research focus views the printed right page more from above while retaining its accepted size; Mac mini focus is higher and closer, with an azimuth that clears the monitor.

### Beosound 9000 and Beolab 8000 — approved 2026-09-11

Replace the former soundbar with the classic 869 × 301 × 70mm silver/black six-CD Beosound 9000 on its near-upright shelf bracket. The floor-standing Beolab 8000 pair uses the documented 132cm height, 17cm square bases, black cloth fronts and polished aluminium taper; no Beolab 2 subwoofer is included. Retain real proportions, physical glass cover, moving clamper and integrated printed controls. A distant click approaches without changing the selected disc; subsequent CD clicks select directly, including while the clamper is moving. No separate generic player panel.

The model provides real playback for the three owner-supplied tracks listed below. Remaining slots stay unassigned and must report that honestly. Audio must be loaded only after an intentional playback action and must use a source permitted for website playback. Never rip or hide a YouTube player to simulate background audio, or imply the owner's streaming subscription can serve all visitors.

### Photographed Bing surfboard — 2026-09-11

The six newly supplied Down photos supersede the earlier generated board graphics. Preserve the actual amber board's separate deck/bottom lettering, 60th-anniversary badge, wax region, rails and single pale fin/fin box. Its confirmed Beacon envelope is 9ft6in × 23.25in × 3in; intermediate rocker and rail profiles remain photograph-based estimates. Use the source-derived lettering, not retyped or fabricated lettering. Originals stay unchanged in Dropbox; the generated GLB and reproducible preparation scripts record provenance.

### Approved audio placement A and reserved cabinet shelves — 2026-09-11

Keep Beosound 9000 horizontal on its current shelf. The Beolab 8000 pair stands outside the two bookcase ends, centered at x = ±2.57m, z = 2.87m. Raise both room-light and roller-blind switch centers to 1.46m, slightly above the speakers; this supersedes the earlier 1.22m rule. Keep their horizontal positions, with blinds left of the window, and retain the nearby controller anchoring.

Leave the Isidoro left leaf's shelf space free of decorative glassware for future bottles. Preserve its actual shelves, rails, seven-bottle collection and lower storage. Glasses and bartender tools remain together on the right half; do not refill the left shelves with generic props.

### Supplied listening tracks and reference details — 2026-09-11

The owner supplied MP3s for CD 1 (Asoto Union, Think About’ Chu) and CD 2 (Brown Eyes, 비오는 압구정). CD 3 now also has the subsequently supplied GIRIBOY track; slots 4–6 remain unassigned. Keep initial entry silent with zero audio fetches until playback intent. One persistent native audio element continues while exploring the room; integrated controls manage play/pause, volume, mute, selection and standby. Reflect real loading, playback and errors, never simulated success. CD selection stops the prior track, moves the clamper, then starts the selected track and disc rotation. Pause restores the original disc orientation; reduced motion suppresses rotation/travel. Display the supplied title discreetly beside the existing operation panel, without adding an album card or global player.

The calendar reserves enough ledger height for the two May events on the reviewed desktop and phone sizes; longer ledgers scroll independently and reset to the top after changing months. Month arrows stay fixed. Keep speaker centers at the approved A positions; move Hue to [-2.63, .0185, 3.18], clear of the right speaker, near the two-wall corner. This preserves rear-wall clearance, not a claim of measured optimal acoustics.

The Bing bottom has the photographed dark amber fan-shaped fin-area patch. Both faces have visibly darker rails following the actual tapered outline. The supplied 9.5-inch anniversary fin photograph supersedes the generic fin: retain its exact printed artwork and subdued fiberglass surface in static texture/UV data, with no per-frame texture generation.

The third supplied track is GIRIBOY — 하루종일 (Band Ver.), from 땡큐 / Thank You. CD 3 uses its embedded digital cover as a custom listening-disc graphic, not a claimed photograph of a commercial CD edition. CD 1 and CD 2 use verified photographs of the actual printed discs. All three native audio sources are encoded AAC160k for delivery and requested only after interaction.

Beacon construction reference: https://bingsurf.com/collections/beacon confirms 9ft6 ×23.25in ×3in (.59055m wide,.0762m thick), broad nose and50/50rails. Both opposing tail-lamination masks contribute their own intermediate amber layer; their intersection forms the dark fan. The deck has a subtle central laminate cap under the wax, peaking near71%of board length. Retain owner-specific Bing and60 graphics. The Flying Diamonds Flex official outline is scaled to the user-confirmed9.5in fin height with a tapered foil and photographed anniversary artwork; the fin and recessed box share the board’s bottom-surface curve.

The 60th-anniversary retaining screw sits on a small tailward tab beside the foil root. The pin and screw orientation follows the owner photograph, which differs from the newer Flex product. The long box and slot surfaces use longitudinal segments so they follow the bottom curvature continuously without disappearing beneath the board or floating above it.
...
### Source-led north-bank buildings — 12 September 2026

Restore all fifteen existing Seobinggo Shindonga slabs on their individual OSM footprints, including the omitted fifth block. Thirteen above-ground floors and the 1984 completion are cross-checked with municipal records. Preserve a pale ivory facade (#d7d1c2), warm white ledges (#e1ddcf), grey-green glazing (#394e4e), quiet gable lettering (#718d9b), and restrained rooftop lift overruns seen in existing-building photographs. The 3.05m storey and small facade dimensions are authored visual estimates; the complex footprint and floor count are source data. Replace the corresponding baked faces rather than stacking duplicate massing.

Restore the three completed Raemian Caelitus towers and their connected seventeenth-floor silhouette on actual mapped footprints. Retain their individually tagged OSM heights and document conflicting published architectural heights without inventing a survey datum. The supplied building inventory carries source URLs and the metric-versus-floor provenance. Nearby small buildings and streets follow saved OSM geometry, preserve parks and the water corridor, and use bounded merged geometry.

Day water uses a restrained grey-green body (#4e655e) with the existing physical sky/building reflection. Night retains #112c3c. Fine normal sampling (18 per world unit into the existing normal function, horizontal strength0.23) avoids oversized bright ripple bands. Preserve the water reflection camera correction, target resolution and time-driven movement. These material values describe this visual scene, not measured river water conditions.

`BanpoAppearance.ts` owns the shared exterior material palette and treatment values; the apartment, tower, street, neighborhood, ground-facing facade and water implementations consume those tokens. Keep the room camera's close-object range separate from the kilometre-scale exterior: the exterior camera uses a 100m near plane and 12,000m far plane so centimetre-separated facade layers remain distinguishable at the actual viewing distance. The camera remains at its established geographic offset; named buildings must never move simply to appear in another window angle.
...
Use the owner's actual Liquor Journal, October 2017, No. 217 cover, photographed contributor note and pages 52–53. A closed 207 × 270 mm softcover magazine leans against the lining in the left opening half of the Isidoro, beside the upper bottle. These dimensions and its 6 mm thickness are visual estimates fitted to the shelf clearance. Selection lifts the closed magazine over the guardrail, pulls it clear, then turns the cover toward the reader. Cover/right-page click opens or advances, left-page click returns; Escape/X closes the cover and reverses the extraction before allowing the cabinet to close. Printed Korean remains source photography; the nearby caption is English and names only the available excerpts. Missing pages are not fabricated.

Reuse photographic quadrilateral UV correction, shared paper materials and room inspection. Turning sheets bend along a segmented, length-preserving arc with correctly oriented backs; turned and remaining stacks conserve total thickness. Motion follows the existing damped scene transition and becomes immediate for reduced motion. No permanent reader panel or unrelated text buttons. The short adjacent caption uses the room editorial font, 16 px desktop / 14 px compact, a 24 px / 16 px gap and warm ivory at night. Small page arrows and X have 44 px hit areas; keyboard arrows and Escape provide the same actions.

The owner's later correction places the exterior and interior Eat Drink & Be Merry photographs immediately after the existing lecture cover, followed by the unchanged lecture pages. They share the same magnet, bending sheets, conserved page-stack thickness and controls. Preserve both full photographs without stretching or cropping, centered with paper margins. No separate photograph is mounted below the lecture. The amplifier proposal frame stays asset-gated until a real poster is supplied; no blank or nonfunctional player appears.

### Archive restoration and scene zoom — 13 September 2026

Print each bar photograph’s caption directly on its magnetic sheet: Eat Drink & Be Merry; Sangsu-dong, Seoul; 14 September 2017 for the exterior and 2 September 2017 for the interior, verified from the originals’ EXIF. Keep the photograph complete above the caption and use the existing editorial/sans font tokens. Cap caption canvases at 1024px on coarse pointers and 1536px elsewhere.

The magazine rests on the shelf and leans about nine degrees into the cabinet lining. Unlean while lifting, then retain the established rail-clearance extraction/return sequence. Reconstruct photographed print blocks separately from source pixels: straighten the photograph/column boundaries, neutralize paper shadows around original ink, remove photographed tabletop and gutter from the printed surface, and soften the contributor excerpt’s cropped outer edge. Do not generate people, substitute text or fabricate missing pages. The original issue was searched but a verifiable full scan was not found. The actual 3D leaves supply the new gentle curvature and turning shadows.

Double-clicking the scene—including unmeshed window/background regions—zooms along the clicked ray. DOM controls, dragging, CV reading and arrangement retain gesture ownership. X/Escape returns to the captured pose.

Magazine reading follows the personal books: a visible binding and paper-edge stack, partially open cover for a lone photographed excerpt, a 165.6-degree article spread, and restrained paper lighting. Frame the actual curved volume, including its forward cover extension.

## Research reader and pinned-paper refinement — 13 September 2026

For visitors comparing papers, shared research context must occupy approximately the top quarter of the reading panel. Combine the editorial title with existing expand/close actions; keep citation metrics, profile links and their checked date compact. Reuse the 24/18/12/11px type scale and existing 4/8/12/16/24px spacing. Tabs and paper arrows share one toolbar with 44px targets. Present the selected paper’s title, authors, citation and abstract before its identifiers and full-page preview. Preserve all actual publication data, source/access links and archive filters. A single reader scrollbar replaces the nested abstract scrollbar. Mobile portrait uses a 48dvh bottom reader and fits the real printed page in the space above it. Short landscape uses a right reader capped at 360px. Keep the title, dated metrics and 44px toolbar fixed; only paper details scroll. The current title and DOI lead into the source abstract, with author/citation details below. Paper changes reset that inner scroll. Profile links remain in All papers and expanded reading. The separate full reading view remains available.

The latest owner correction supersedes the single upper-left magnet and leftward paper turn. Two small stationary nickel magnets clamp the top strip. Lift the free lower edge and curl the sheet upward around a finite-radius fold below that strip. The sheet and both transferred page stacks must remain physically outside the cabinet seam; the magnets obey real depth occlusion, never overlay the moving sheet. Settled leaves retain slight curvature, with conserved sheet thickness. Match photograph captions to the existing Georgia/Manrope editorial hierarchy and paper/ink/muted palette; keep the real photograph and date. The closed cabinet approach is lower and closer, with the complete cabinet visible and its paper/handle sightlines clear of the desk. These are geometric approximations, not a cloth simulation; preserve reduced-motion and keyboard controls.

The selected paper places its underlined DOI directly below the title, with a 44px minimum link target that opens the original in a new tab.

### Proposal recording on the amplifier — 13 September 2026

One small, supported black photograph frame rests on the Fender amplifier. Its still is the exact 23-second frame from the owner's final proposal video; preserve the complete 16:9 composition. Clicking the frame approaches it and starts the recording on the same physical screen. Camera navigation alone never starts media. The nearby archive caption uses the shared room typography, day/night ink and measured object placement, with a short personal story and 44px playback targets. Keep play/pause, position, volume/mute and an optional fullscreen view beside the object; no persistent global video bar.

Load the recording and its streaming support only after deliberate playback. Serve adaptive 720p/1080p H.264/AAC segments with a compact MP4 compatibility source, retaining the untouched master outside the site. Closing, changing objects or entering arrangement releases the media decoder, requests and video texture, restores the still, and resets playback. The proposal and Beosound pause one another so their audio never overlaps. Reduced-motion navigation and keyboard activation use the same lifecycle.

### Music corner navigation repair — 13 September 2026

Clicking either the physical guitar or amplifier approaches the shared corner from the open side of the desk, without loading or playing the recording. A second deliberate click on the small frame begins playback. The frame faces with the amplifier and rests on its top beside the handle, with a supported easel. Closing the recording returns to the corner when entered through it; closing the corner returns to the previous room view. Direct frame entry preserves direct return. Keep touch/drag discrimination, keyboard activation, Escape, and viewport fitting.
...
### Seated desk entry and room reveal — 2026-09-13

The owner's latest correction replaces the whole-room loading still with a seated-at-desk view. The first frame must include the entire physical CV monitor, keyboard and desktop, plus the background room and TV. It must not be a fullscreen CV, enlarged standalone bezel, or a simulated document page. Generate the responsive day/night stills from the same seated camera pose and projection used by the real scene; retain actual materials, objects and source CV content.

Once real scene assets have rendered at the matching seated pose, replace the still and pull the real camera continuously back to the approved rear-quarter Overview. Use the existing4.5/second camera damping and180ms opacity interval; add no progress simulation, minimum loading dwell, animation library, media autoplay or repeated intro. The reveal is a project-specific3D extension of the beui.dev center-morph-modal's continuity, independent opacity and interruption mechanics (https://beui.dev/r/center-morph-modal/raw, inspected2026-09-13). Reduced motion switches directly to Overview after readiness without camera travel.

The existing Read CV action remains available before WebGL is ready and opens the native CV reader. Choosing it records reading intent: do not dismiss that reader or reset its scroll when the room becomes ready. Overview/Explore room explicitly releases reading intent and enters the room. Pointer, keyboard, touch and alternate navigation remain available; there is no focus trap for passive entry. Any deliberate room navigation supersedes automatic reveal. Exhibit deep links and poster-capture requests bypass the intro; capture requests can select the seated pose without automatic pullback. Resizing recomposes the active seated or overview pose and never restarts a completed intro. Preserve the native CV geometry/typography and all close-up cameras, first-screen instrument sightlines, explicit playback and renderer frame scheduling.

## 2026-09-13 — Additional photographed honors
Five owner-supplied objects use the existing shelf inspection and unboxed caption system: Neurospine Outstanding Reviewer Award (4 September 2025), SNU master's degree congratulatory wooden plaque (26 February 2018), KOSESS academic award (23 August 2025), WCMISST invited-speaker certificate (7–9 May 2026), TSESS instructor certificate (9 June 2026). Preserve all photographed text, logos, signatures and seals. Rectify only perspective using source paper/metal corners; do not redraw their content. Documents use the existing satin-aluminium frame, mat and rear support. The SNU plaque retains dark stepped wood, an inset metallic face, routed rear recess and ribbed silver support. Left top shelf holds the three honors; right top shelf holds the two teaching certificates. Existing book and award locations stay intact. A4 is the working paper scale; plaque dimensions are photograph-based estimates. Avoid additional transmission passes or per-frame effects.

### Additional honor print repair — 13 September 2026
The owner's rejection supersedes the perspective-only preparation above. Keep the original photographed ink, signatures and seals; reject generative substitutions. Map the measured complete paper/metal boundary onto a genuinely flat surface, correcting the source projection independently of the physical frame. Calibrate photographed illumination out of the print material so the scene supplies its light; retain intentional printed gray bands and watermarks. The metal face must not carry the photographer's reflected silhouette. The wood body, trim, metal substrate and printed pigments remain separate materials. Validate each of five surfaces unlit and under changing scene light before release. The thank-you card remains excluded.

### CD library — 14 September 2026
A low graphite rack with six supported jewel cases sits beside the horizontal Beosound. Reuse its silver/black materials and the existing original album covers. The six owner-supplied tracks remain in their chosen order on first entry. Case selection opens a small nearby paper/ink panel; select a player slot, insert/move a disc or return that slot's disc to its case. Each album has one physical disc: moving it leaves its previous slot empty. Empty slots are skipped by transport and automatic continuation. Replacing a disc stops playback; Play remains deliberate. No upload, invented albums, global player, or persistent storage in this pass.
Reuse labelled 44px controls, self-hosted body type, paper/ink/clay palette, 4px radius, 180ms opacity and keyboard Escape/focus conventions. The beui.dev select mechanism informs scoped selection and reduced-motion continuity; use native controls without adding a motion dependency. Physical transfer is a novel 3D sequence: open glass, lift the outgoing disc into its case, carry the chosen disc into the slot, close glass. Block overlapping transfer commands; closing inspection does not abort an in-flight physical move. Reduced motion commits immediately. Load no additional audio buffers and reuse album textures; moving meshes exist only during transfer.

### Browsable CD collection — 14 September 2026
Supersedes the graphite rack and single-track selector above. A Resaw-inspired slanted walnut tabletop holder supports up to 18 jewel cases, 28cm wide, 16cm deep and 15cm high. Cases stand side by side with visible labelled spines; browsing fans them slightly and pulls the selected case toward the visitor. Original covers only; the first six discs retain the owner's order, with NELL Separation Anxiety, Maroon 5 Songs About Jane and John Mayer Continuum added to the collection. These three have metadata and physical discs but no playable file until supplied.
The nearby album booklet uses an ivory cover beside a dark ink track insert: ink #202D2A, ivory #F8F6F0, readable unavailable tracks #A7AEA8, clay selection. Manrope 14px/1.65 body, 11–12px metadata, Georgia 24px album title, 4/8/12/16/24px spacing, 44px targets. A horizontal case rail supports pointer drag, touch swipe, arrow keys and labelled previous/next buttons. Reference: beui.dev cylinder-carousel source consulted for bounded selection, pointer capture and reduced-motion behavior; use the existing R3F interpolation and native CSS transforms without another animation dependency. No decorative bounce or idle animation.
On desktop the booklet is below the player, on portrait mobile a bounded lower sheet leaves the player visible above; short landscape uses a compact two-column insert. Tracks are actual release order, with file availability separate from metadata. Loaded albums retain empty cases and a slot label. Mount-only versus play-track intent is explicit. A physical swap lasts 1.4s including a restrained lid interval; other-slot mount-only exchanges retain current audio and use closed-cover abstraction. Commit the unique six-slot map only at completion. Last queued user intent wins; pause cancels queued playback. Persist a validated versioned slot array locally, never playback intent. An unobtrusive title/pause control remains while listening outside inspection. Reduced motion commits immediately. Verification must include seventh-album exchange, unavailable tracks, denied media/storage, native ended continuation, queued choices, persisted reload, mobile bounds and keyboard navigation.

### CD collection browsing (2026-09-14)
The room remains visible during album browsing. Cover Flow uses transparent, unframed navigation with only covers, a compact selected-album caption and text actions. No background sheet, tray illustration, persistent track list, or duplicate playback toolbar. Selecting the centered cover reveals its track list on request; Escape dismisses that list before leaving the collection. Cover motion retains the existing 380ms transform transition and reduced-motion fallback.

### Global music control — 14 September 2026 revision
The only global music button is the first item in the existing upper-right toolbar, immediately before Overview, time/light and Arrange. Reuse that toolbar's 44px target, 12px corner, thin border, translucent surface and 18px outline glyph. No bottom music row, title button or separate player box. During confirmed playback, artist/title scroll left in a single 200px-wide line 6px above the button; start at the music button's left edge and keep within the toolbar/right viewport edge. Use two identical text runs, CSS transform-only linear motion (at least12s, scaled for title length), hidden duplicate for accessibility, hover pause, and stationary ellipsis for reduced motion. Pause/loading/error hide the title; loading remains cancellable with pause. Keep the existing audio engine and physical CD state. Reference mechanism: beui.dev/r/marquee/raw (duplicated runs, translate loop, overflow mask); no new animation dependency.

### Mobile loading refinement · 2026-09-14
The seated monitor composition remains the opening. Loading contains no CV/explore buttons or boxed status. A single 12px Manrope line, “Opening the office…”, sits above the projected top-center of the monitor, clearing its physical light bar by a gap of max(16px, 7% of projected monitor width), using the same responsive crop geometry as the poster. It is visible in server HTML before hydration. No added progress animation or spinner; ready fades the poster using the existing micro transition, reduced motion is immediate. Failure replaces the line with a plain reload message; normal header routes remain usable. Mobile posters are captured at 2x device resolution (build time only); runtime mobile DPR remains capped at 1. The room overview moves 12% closer on mobile and 8% closer on desktop along the existing viewing direction. Reference: existing room typography and beui loader's status/reduced-motion mechanics.

### Beosound native interaction · 2026-09-14
The physical faceplate remains visible and operable in every camera state. No duplicate HTML player, mobile controller box or CD-player chooser. At room distance a tap approaches without playback; after approach a single tap on a disc or printed key performs that action. Touch regions and lettering share one coordinate table. Only CD is renamed PLAY by explicit user request; PAUSE, MUTE, VOL, LOAD and standby follow the existing device. Hover/keyboard focus lightly highlights the physical key. Screen-reader buttons invoke the same commands without a visible duplicate panel. Rack Cover Flow and the upper-right global playback icon remain. Preserve transport/cover animation, gesture drag suppression and audio lifecycle. Official reference: https://bangolufsenassistentgohe.blob.core.windows.net/manuals/SOUND_SYSTEMS/BEOSOUND_9000/beosound9000_userguide_english.pdf .
...
## Room-only navigation and album framing — 2026-09-15
Home ends at the room viewport; remove the below-room publication teasers, footer and Browse the work link. Keep scrolling within readers. Guided destinations distinguish Research (desk folio), Talks & Recognition (TV and awards), UBE Training, Music & Stories, and Jiu-jitsu & Surfing (gi and board together). Preserve existing hover treatments; otherwise use a restrained warm material accent on mouse hover only, restored on leave, press and Arrange. No extra light, bloom, idle animation or touch hover.
Research keeps one clickable DOI destination with separate Cite and available PDF actions. Remove duplicate publisher/PDF links without changing access metadata. Desktop album browsing backs off to retain the complete player and rack above transparent CoverFlow; use 20px album names and 14px metadata/actions, ivory lettering with a dark text shadow rather than an opaque panel.

Proposal video controls use shared outline Play/Pause and speaker icons with 44px targets. Volume appears vertically above the speaker on hover or tap, supports keyboard range keys, mute, outside dismissal and Escape. Keep the platform-volume fallback on iOS, and keep an icon-only fullscreen button at the video's lower-right. Preserve the existing playback engine and seek bar.

Residential window lighting follows an interpretive Seoul-time schedule: existing lit windows diminish from 22:00 (100%) to midnight (54%) and 02:00 (8%), stay sparse until 05:00 and recover toward morning as daylight suppresses emission. Stable per-window seeds stagger switching with short fades; occupancy changes update one shared shader uniform at 30-second resolution. Roads, bridges and landscape lighting retain their independent night behavior. No extra draw calls, timers or live-residence claims.

### Personal painting and desk clearance — 2026-09-15
The original painting replaces the calendar treatment. Hang a 62 cm square oak frame above the garment rack, centered on the coat and gi, facing into the room. Preserve the full source image with an ivory mat; no calendar grid or owner portrait. Reuse the room inspection and quiet hover behavior with a short artist credit. Move each desk side frame 4 cm outward, preserving the tabletop and accessories, to increase inner clearance by 8 cm.
...
## September 25 — SZQ textured canvas

Display the owner's December 10, 2020 SZQ Gallery painting beside the existing 2018 work above the garment rack. Following the owner's size correction, use a 42 × 56 × 4 cm canvas beside a 42 cm square calendar frame: comparable widths, with the newer canvas visibly taller. These are display estimates, not measured dimensions. The pair spans .96 m, centered on the rack, with a 12 cm clear gap and common 2.35 m center height. Preserve the original calendar painting's frame and orientation, and the canvas's original white edge, heart, paint texture, and handwritten date; no invented frame or glass. Source-preserving registration and restrained high-pass bump/displacement bring out the matte paint without changing its color composition. Reuse archivePose, RoomArchiveCaption, PALETTE.paperLight, and the existing material-hover interaction; pointer, keyboard, X, Escape, Overview, and responsive placement follow the existing artwork behavior. Show the owner's original photograph holding the paintings with his wife alongside the caption, with the existing RoomPhotoGallery enlargement and focus-return behavior. A two-column text/photo caption reuses the 16px gap and existing typography; the photograph loads only when the artwork is opened. Bring the Work & Life Balance guided camera closer while keeping both pictures, the rack and surfboard in view. Asset provenance accompanies the WebP textures and photograph.
